              ...
              <div class="grid-content">
                <div class="info-saide">
                  ...
                  <ul>
                    ...
                    <li><a :class="{'active': panel_id=='position'}" href="javascript:;" @click="panel_id='position';filters.page=1"><i class="el-icon-document-checked"></i> 职位</a></li>
            ...
            <el-col :span="14">
              <div class="grid-content">
                ...
                <div class="info-content" v-if="panel_id == 'collect'">
                  <Collections v-for="(item, i) in collect_list.results" :key="i" v-bind:item="item" v-bind:userInfo="userInfo"></Collections>
                  <el-pagination
                    background
                    @current-change="handleCurrentChange"
                    layout="prev, pager, next"
                ...
                <div class="info-content" v-if="panel_id == 'position'">
                  <a class="create" :href="`/createposition/${this.user_id}`" target="_blank"><i class="el-icon-circle-plus-outline"></i> 发布职位</a>
                  <div class="position-wrapper">
                    <Position @removeItem="refresh_list()" v-for="(item, i) in position_list.results" :key="i" v-bind:item="item"></Position>
                  </div>
                  <el-pagination
                    background
                    :pageSize="6"
...
export default {
  ...
  methods: {
    ...
    async getData () {
      ...
      if (this.panel_id == 'position') {
        const response = await this.axios.get(`${this.settings.Host}/home/${this.panel_id}/?publisher=${this.user_id}&companyId=${this.company_id}`, {
          params: this.filters
        });
  ...
  components: {
    News, Collections, Articles, Position
  }
};
</script>
