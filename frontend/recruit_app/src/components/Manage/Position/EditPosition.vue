            ...
            <el-form :model="PositionRequire" ref="PositionRequireForm" label-width="100px" class="demo-ruleForm">
                ...
                >   
                    <el-tooltip class="item" effect="dark" content="例：10-20。以K（千）为单位" placement="top">
                        <el-input type="text" v-model="PositionRequire.salary" autocomplete="off"></el-input>
                    </el-tooltip>
                </el-form-item>
                <el-form-item
                    label="工作地点"
                ...
                <el-form-item>
                    <el-button type="primary" @click="editPosition">发布</el-button>
...
export default {
    ...
    created () {
        ...
        this.positionRetrieve(this.position_id);
    },
    methods: {
        // 绑定@imgAdd event
        imgAdd(pos, $file){
        ...
        // 获取单个职位信息
        positionRetrieve(position_id) {
            this.axios.get(`${this.settings.Host}/home/update/position/${position_id}/`)
            .then(response => {
                console.log(response)
                this.PositionRequire = response.data;
                this.PositionRequire.jobNature = String(response.data.jobNature);
                this.PositionRequire.isSchoolJob = String(Number(response.data.isSchoolJob));
                this.PositionRequire.isSchoolJob = String(Number(response.data.isSchoolJob));
                this.PositionRequire.PositionType = Array(response.data.firstType, response.data.secondType, response.data.thirdType);
                let city = response.data.city.split('-');
                this.PositionRequire.PositionArea = Array(city[0], city[1], response.data.district);
            });
        },
        editPosition() {
            this.axios.post(`${this.settings.Host}/home/update/position/${this.position_id}/`, {
                    publisher: this.user_id,
                    companyId: this.company_id,
                    positionName: this.PositionRequire.positionName,
                    salary: this.PositionRequire.salary,
